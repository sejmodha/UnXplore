...
DIRS, SAMPLES = glob_wildcards(config['data']+"/{dir}/{sample}.fastq.gz")
#print(DIRS)
...
rule all:
    input:
        #expand(config['data']+'/{dir}/{sample}_trim.fastq.gz',zip,dir=DIRS,sample=SAMPLES),
        #expand(config['data']+'/{dir}/{sample}_GRCh38mapped.bam',zip,dir=DIRS,sample=SAMPLES),
        #expand(config['data']+'/{dir}/{sample}_GRCh38_unmapped.fastq.gz',zip,dir=DIRS,sample=SAMPLES),
...
rule trimming:
    input:
        config['data']+'/{dir}/{sample}.fastq.gz'
    output:
        temp(config['data']+'/{dir}/{sample}_trim.fq.gz')
    params:
    ...
    shell:
        ...
        ktrim=r k=23 mink=11 hdist=1 qtrim=rl trimq=20 tpe tbo t={threads}
        """

rule human_unmapped:
    input:
        config['data']+'/{dir}/{sample}_trim.fq.gz'
    output:
        bam = config['data']+'/{dir}/{sample}_GRCh38mapped.bam',
        r = temp(config['data']+'/{dir}/{sample}_GRCh38_unmapped.fastq')
    params:
        ref = config['genome']
    threads: 12
    conda:
    ...
    shell:
        ...
        samtools fastq -@ {threads} -f 4 {output.bam} > {output.r}
        """

rule bbmap_diginorm:
    input:
        config['data']+'/{dir}/{sample}_GRCh38_unmapped.fastq',
    output:
        temp(config['data']+'/{dir}/{sample}_GRCh38_unmapped_diginorm.fq.gz')
    params:
        target=100,
    ...
    shell:
        """
        bbnorm.sh in={input} out={output} target={params.target} hist={params.hist} histout={params.histout} t={threads} mindepth={params.mindepth}
        """

rule spades_assemble:
#Note that spades assembly for single end reads is not --meta as metaSPAdes does not support single end reads
    input:
        config['data']+'/{dir}/{sample}_GRCh38_unmapped_diginorm.fq.gz'
    output:
    ...
    shell:
        """
        spades.py -s {input} -o {params.outdir} -m {params.ram}
        """

rule bwa_contig_map:
    input:
        ref = config['data']+'/{dir}/{sample}_spades/contigs.fasta',
        r = config['data']+'/{dir}/{sample}_GRCh38_unmapped.fastq'
    output:
    ...
    shell:
        ...
        bwa mem {params.tempref} {input.r} |samtools view -b -h | samtools sort -o {output} -
...
rule get_assembly_stats_contigs:
    ...
    shell:
        """
        pileup.sh in={input} ref={params.contigs} out={output} 32bit=t overwrite=t
        """
...
rule extract_reads_stats:
    input:
        fastq = config['data']+'/{dir}/{sample}.fastq.gz',
    ...
    shell:
        """
        python3 {params.get_reads_stats} -f1 {input.fastq} -u {input.humanbam} -c {input.contigbam} -r {params.runid} -b {params.bioproject} -o {output} -t {threads}
        """
