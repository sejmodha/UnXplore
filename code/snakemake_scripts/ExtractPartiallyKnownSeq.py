...
def extract_hits(inputfile,outputfile,percid):
    ...
    #Exclude rows where pident could be -1 as those rows represent dark contigs records
    df=df[(df['pident'] > 0)]
    subset_df=get_max_rows(df)
    print(f'Percent ID is {percid}')
...
extract_seqs(extract_hits(indiam,outdiam,percid=80),infasta,outfasta)
